...
CUBE_START_POS = np.array([-0.62, -0.18, BLOCK_CENTER_Z], dtype=np.float64)
CUBE_PLACE_POS = np.array([-0.62, 0.28, BLOCK_CENTER_Z], dtype=np.float64)

# Lower target heights.
# These are attachment_site targets, not cube centre targets.
# If the robot looks too low or collides, increase PICK_DOWN_POS / PLACE_DOWN_POS from 0.105 to 0.12.
PICK_APPROACH_POS = np.array([CUBE_START_POS[0], CUBE_START_POS[1], 0.24], dtype=np.float64)
PICK_DOWN_POS = np.array([CUBE_START_POS[0], CUBE_START_POS[1], 0.110], dtype=np.float64)

PLACE_APPROACH_POS = np.array([CUBE_PLACE_POS[0], CUBE_PLACE_POS[1], 0.24], dtype=np.float64)
PLACE_DOWN_POS = np.array([CUBE_PLACE_POS[0], CUBE_PLACE_POS[1], 0.150], dtype=np.float64)
...
def create_scene_xml() -> str:
    ...
    xml = f"""
<mujoco model="ur10e_pick_place_v2_demo_lower_targets">
    ...
    <worldbody>
        ...
        <site name="pick_approach_marker"
              pos="{PICK_APPROACH_POS[0]} {PICK_APPROACH_POS[1]} {PICK_APPROACH_POS[2]}"
              size="0.012"
              rgba="1 0 1 1"/>

        <site name="pick_down_marker"
              pos="{PICK_DOWN_POS[0]} {PICK_DOWN_POS[1]} {PICK_DOWN_POS[2]}"
              size="0.010"
              rgba="1 0.4 1 1"/>

        <site name="place_approach_marker"
              pos="{PLACE_APPROACH_POS[0]} {PLACE_APPROACH_POS[1]} {PLACE_APPROACH_POS[2]}"
              size="0.012"
              rgba="0 1 1 1"/>

        <site name="place_down_marker"
              pos="{PLACE_DOWN_POS[0]} {PLACE_DOWN_POS[1]} {PLACE_DOWN_POS[2]}"
              size="0.010"
              rgba="0.4 1 1 1"/>

        <site name="mid_marker"
              pos="{MID_POS[0]} {MID_POS[1]} {MID_POS[2]}"
              size="0.010"
              rgba="1 1 0 1"/>
...
def get_cube_pos(model, data) -> np.ndarray:
    body_id = name_to_id(model, mujoco.mjtObj.mjOBJ_BODY, "red_cube")
    return data.xpos[body_id].copy()


def set_cube_pos(model, data, pos: np.ndarray):
    p = np.asarray(pos, dtype=np.float64).copy()
    p[2] = max(p[2], BLOCK_CENTER_Z)
    set_freejoint_pose(model, data, "red_cube_freejoint", p)
    mujoco.mj_forward(model, data)
...
def build_waypoints(model, data):
...
def hold_pose(model, data, viewer, duration=0.4, realtime=True, attached=False, attach_offset=None):
    ...
    for _ in range(steps):
        ...
        if attached:
            ee_pos = get_ee_pos(model, data)
            set_cube_pos(model, data, ee_pos + attach_offset)
...
def run_pick_place(model, data, viewer, waypoints, realtime=True):
    print("")
    print("========================================")
    print("Running v2 lower-target pick-and-place demo")
    print("One red cube: green pick area -> blue place area")
    print("========================================")

    set_cube_pos(model, data, CUBE_START_POS)
    ...
    set_cube_pos(model, data, CUBE_PLACE_POS)
    ...
    final_cube = get_cube_pos(model, data)
    place_error = np.linalg.norm(final_cube[:2] - CUBE_PLACE_POS[:2])

    print("")
    print("========================================")
    print("Pick-and-place finished")
    print("Final cube pos:", np.round(final_cube, 3))
    print("Place target:", np.round(CUBE_PLACE_POS, 3))
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Clean UR10e MuJoCo v2 lower-target pick-and-place demo."
    )
...
def main():
    ...
    print("Temporary v2 lower-target pick-place scene:", scene_path)
    ...
    waypoints = build_waypoints(model, data)

...
# Fixed starting positions for now
...
# Target heights
# ============================================================
# These are the final tuned heights you chose.
...
def create_scene_xml(robot_target=None, human_target=None) -> str:
    ...
    for color in BLOCK_COLORS:
        ...
        <geom name="{color}_place_slot"
              ...
              rgba="{COLOR_RGBA[color].replace('1"', '0.35"')}"
              contype="0"
              conaffinity="0"/>
        """

    # Debug target markers
    marker_xml = ""
    ...
    if human_target is not None:
        ...
        <site name="human_target_marker"
              pos="{human_pos[0]} {human_pos[1]} 0.15"
              size="0.014"
              rgba="1 1 0 1"/>
        """

    xml = f"""
<mujoco model="ur10e_pick_place_v3_multiblock">
...
def choose_robot_target(human_target: str) -> str:
    """
    Choose a non-conflicting cube.
    Current simple rule:
    choose the first available colour that is not the human target.
    """
...
def hold_pose(model, data, viewer, duration=0.4, realtime=True, attached=False, attach_offset=None, color=None):
...
def run_pick_place(model, data, viewer, waypoints, human_target, robot_target, realtime=True):
    print("")
    print("========================================")
    print("Running v3 multi-block pick-and-place demo")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="UR10e MuJoCo v3 multi-block non-conflicting pick-and-place demo."
    )
    ...
    parser.add_argument(
        ...
        help="Human intended cube colour."
    )
...
def main():
    ...
    robot_target = choose_robot_target(human_target)
    ...
    print("Temporary v3 multiblock scene:", scene_path)

...
# Distance range for early prediction, in meters.
EARLY_MIN_DISTANCE_M = 0.025
EARLY_MAX_DISTANCE_M = 0.260

# Recent hand displacement threshold, in meters/window.
MIN_HAND_SPEED_M = 0.006

# Confirmation rules.
CONFIDENCE_THRESHOLD = 0.42
STABLE_TIME_REQUIRED = 0.28

# Smaller value makes the highest probability more obvious.
SOFTMAX_TEMPERATURE = 0.65

# Distance shaping.
DISTANCE_SIGMA_M = 0.09
...
def extract_hand_points(frame, hand_landmarks):
    ...
    # Main intention point: index fingertip.
    ...
    hand_point_table = index_tip_table
...
def predict_intention(hand_table, hand_velocity_table, finger_direction_table):
    ...
    for color in candidates:
        ...
        # Exponential form gives stronger separation than linear distance.
        ...
        closing_score = float(np.clip(distance_change / 0.035, 0.0, 1.0))

        # Early prediction factor:
        # Too close is late-stage grasping, too far is uncertain.
...
def update_stable_lock(best_color, best_prob, best_distance_m, hand_speed_m, details):
    ...
    # If finger direction is strong, allow lower speed.
    speed_ok = hand_speed_m >= MIN_HAND_SPEED_M or finger_score > 0.55
    ...
    approaching = (
        ...
        or closing_score > 0.08
        or finger_score > 0.45
    )
